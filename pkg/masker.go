...
func (m *randomMasker) Mask(value any) any {
	if value == nil {
		return nil
	}
	...
	case string:
		...
		for _, layout := range layouts {
			if t, err := time.Parse(layout, v); err == nil {
				year := m.f.IntRange(2000, 2039)
				month := time.Month(m.f.IntRange(1, 12))
				day := m.f.IntRange(1, 28)
		...
		if numberLikeRegex.MatchString(v) {
			...
			for _, char := range v {
				if char >= '0' && char <= '9' {
					result.WriteString(strconv.Itoa(m.f.IntRange(0, 9)))
				} else {
					result.WriteRune(char)
				}
			}
		...
		if _, err := net.ParseMAC(v); err == nil {
			return m.f.MacAddress()
		}
		if ip := net.ParseIP(v); ip != nil {
			if ip.To4() != nil {
				return m.f.IPv4Address()
			}
			return m.f.IPv6Address()
		}

		if _, err := strconv.ParseInt(v, 10, 64); err == nil {
			return m.f.Numerify(strings.Repeat("#", len(v)))
		}
		if _, err := strconv.ParseFloat(v, 64); err == nil {
			parts := strings.Split(v, ".")
			integerPart := parts[0]
	...
	case json.Number:
		...
		if strings.Contains(s, ".") {
			...
			fractionalPart := parts[1]
			template := strings.Repeat("#", len(integerPart)) + "." + strings.Repeat("#", len(fractionalPart))
			return json.Number(m.f.Numerify(template))
		}
		return json.Number(m.f.Numerify(strings.Repeat("#", len(s))))
