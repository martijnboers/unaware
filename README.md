...
**Goal:** Mask all sensitive user details and the session IP address, but leave the user's name untouched for reference.
...
- `-exclude 'user.personal_info.name'` then removes the name from that list, even though it was included by the wildcard.
...
```json
{
  "user": {
    ...
    "personal_info": {
      "name": "Jane Doe",
      "email": "alfredofritsch@dickinson.net"
    }
  },
  "session": {
