...
func TestXMLProcessing_Structured(t *testing.T) {
	...
	err = pkg.Start("xml", 1, bytes.NewReader(inputWithHeader), &buf, pkg.Deterministic(salt), nil, nil)
	require.NoError(t, err)
...
func TestXMLStreaming(t *testing.T) {
	...
	var buf bytes.Buffer
	err := pkg.Start("xml", 1, strings.NewReader(input), &buf, pkg.Deterministic(salt), nil, nil)
	require.NoError(t, err)
